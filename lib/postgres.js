...
PG.prototype.query2 = function (sql, params, callback) {
    var time = Date.now();
    var log = this.log;
    ...
    queries.forEach( function(query, index) {
        if (index !== queries.length-1) {
            ...
            self.client.query(query, params, function (err, data) {
                if (log) log(sql, time);
                callback(err, data ? data.rows : null);
            });
        }
...
PG.prototype.create = function (model, data, callback) {
    ...
    sql += ' RETURNING id';
    this.query2(sql, values, function (err, info) {
...
PG.prototype.updateOrCreate = function (model, data, callback) {
    ...
    Object.keys(data).forEach(function (key) {
        if (props[key] || key === 'id') {
            ...
            var v;
            if (key !== 'id') {
                v = pg.unScapedToDatabase(props[key], data[key], values);
            } else {
                v = data[key];
            }
            fieldsNames.push(k);
    ...
    this.query2([update, insert], values, function (err, info) {
    ...
    if(forCreate){
      ...
      Object.keys(data).forEach(function (key) {
          if (props[key]) {
              if (key === 'id') return;
              columns.push('"' + key + '"');
              fields.push(this.unScapedToDatabase(props[key], data[key], valuesArray));
          }
      }.bind(this));
      return '(' + columns.join(',') + ') VALUES ('+fields.join(',')+')';
    }else{
      Object.keys(data).forEach(function (key) {
          if (props[key]) {
              fields.push('"' + key + '" = ' + this.unScapedToDatabase(props[key], data[key], valuesArray));
          }
      }.bind(this));
      return fields.join(',');
    }
...
PG.prototype.unScapedToDatabase = function (prop, val, filterValues) {
    if (val === null) {
        // Postgres complains with NULLs in not null columns
        // If we have an autoincrement value, return DEFAULT instead
        if( prop.autoIncrement ) {
...
PG.prototype.all = function all(model, filter, callback) {
    ...
    this.query2('SELECT ' + this.getColumns(model) +  '  FROM ' + this.tableEscaped(model) + ' ' + processedFilter.filterExp, processedFilter.values, function (err, data) {
        if (err) {
            console.log(err);
            return callback(err, []);
        }
...
PG.prototype.toFilter = function (model, filter) {
    ...
    if (filter.where) {
        ...
        Object.keys(conds).forEach(function (key) {
            ...
            if (props[key]) {
                var filterValue = this.unScapedToDatabase(props[key], filter.where[key], filterValues);
                if (filterValue === 'NULL') {
...
function getTableStatus(model, cb){
    ...
    this.query2('SELECT column_name as "Field", udt_name as "Type", is_nullable as "Null", column_default as "Default" FROM information_schema.COLUMNS WHERE table_name = $1', [this.table(model)], decoratedCallback);
};
